...
def test_risk_metrics_match_known_sample():
    ...
    assert metrics.var_95 == pytest.approx(np.percentile(returns, 5))
    assert metrics.cvar_95 == pytest.approx(returns[returns <= metrics.var_95].mean())
    assert metrics.sharpe_ratio == pytest.approx(np.sqrt(252) * returns.mean() / returns.std())
    downside = returns[returns < 0]
    assert metrics.sortino_ratio == pytest.approx(np.sqrt(252) * returns.mean() / downside.std())
    assert metrics.max_drawdown == pytest.approx(abs(((equity - equity.cummax()) / equity.cummax()).min()))
...
def test_performance_metrics_use_trade_pnl_values():
    ...
    metrics = PerformanceAnalyzer.analyze(equity, trades, initial_capital=1000)
