...
def _order_to_dict(order) -> dict:
    ...
    return {
        ...
        "symbol":        order.symbol,
        "direction":     direction,
        "direction_label": {"long": "多", "short": "空", "net": "净"}.get(direction, direction),
...
def _build_system_snapshot() -> dict:
    ...
    return {
        ...
        "trading_day":         str(connection.get("trading_day", "")),
...
def create_app(title: str = "量化交易系统 API", version: str = "1.0.0") -> FastAPI:
    ...
    def get_system_status():
        ...
        return SystemStatusResponse(
            ...
            td_connected     = bool(connection["td_connected"]),
            md_connected     = bool(connection["md_connected"]),
            contracts_ready  = bool(connection["contracts_ready"]),
            reconciliation_ready = bool(connection["reconciliation_ready"]),
            trading_day      = str(connection["trading_day"]),
            order_entry_ready = bool(connection["order_entry_ready"]),
            gateway_status   = gateway_status,
            gateway_name     = gateway_name,
            reconnecting     = bool(connection["reconnecting"]),
            reconnect_count  = int(connection["reconnect_count"]),
